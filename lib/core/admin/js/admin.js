...
function startEditing() {
	...
	var readyFrame = function($frame) {
		...
		$pageContents.find('.fcmsEditable').on('focus', function() {
			changes = true;
		}).on('blur', function() {
			if($(this).text() === '') {
				$(this).html('');
		...
		$pageContents.find('.fcmsEditable[data-type="plain"]').attr('contenteditable', true).on('paste', function(e) {
		...
		$pageContents.find('.fcmsEditableImage .fcmsButton').click(function(e) {
			e.preventDefault();
			e.stopPropagation();
