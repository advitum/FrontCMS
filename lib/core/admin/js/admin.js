...
$(document).ready(function() {
	...
	$(document).on('keyup', function(e) {
		var $buttons = $('.lightbox:last .box .buttonbar');
		if($buttons.size()) {
			if(e.which === 13 && $buttons.find('.fa-check').size()) {
				$buttons.find('.fa-check').closest('button').click();
			}
			if(e.which === 27 && $buttons.find('.fa-times').size()) {
				$buttons.find('.fa-times').closest('button').click();
			}
		}
	});
	...
	$('#pageTree').on('contextmenu', 'ul a', function(e) {
		...
		if(!isRoot) {
			if(isDeleted) {
				...
				items.push({
					title: languageString('Delete permanently'),
					callback: function() {
...
function pageSave() {
	if($('body').hasClass('edit')) {
		...
		$('#page').contents().find('.fcmsEditable').each(function() {
			...
			if($(this).data('global') || $(this).closest('.fcmsFlexlist').data('global')) {
				$form.append('<input type="hidden" name="globals[]" value="' + $(this).data('name') + '" />');
			}
		});
...
function startEditing() {
	...
	var readyFrame = function($frame) {
		...
		var scripts = [
			adminUrl + 'js/languagestring.js?v=' + version,
			adminUrl + 'js/jquery-1.11.2.min.js?v=' + version,
			adminUrl + 'js/tinymce/tinymce.min.js?v=' + version,
			adminUrl + 'js/tinymce/jquery.tinymce.min.js?v=' + version,
			adminUrl + 'js/tinymce.fcmslink.js?v=' + version,
			adminUrl + 'js/inject.js?v=' + version
		]
		...
		$pageContents.on('click', '.fcmsEditablePlugin .fcmsButton', function(e) {
			...
			var meta = {
				'_meta[plugin]': $plugin.data('plugin'),
				'_meta[name]': $plugin.data('name'),
				'_meta[content]': $plugin.attr('data-content'),
				'_meta[attributes]': $plugin.attr('data-attributes')
			};
			...
			ajax('plugin_edit', {}, {
				...
				success: function(response) {
					if(response.success) {
						box(response.content, {
							'<i class="fa fa-check">': function() {
								var data = $('.box').last().find('form').serializeArray();
						...
						ajax('user_add', {}, {
							success: function(response) {
								if(response.success) {
									...
									$('#user-add').replaceWith(response.content);
									boxFunctions();
								} else {
									ajaxError(response);
								}
